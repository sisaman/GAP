...
class AddTrainValTestMask:
    ...
    def __call__(self, g: dgl.DGLGraph):
        ...
        for _ in range(self.num_splits):
            train_mask, val_mask, test_mask = self.__sample_split__(g)
            train_masks.append(train_mask)
            val_masks.append(val_mask)
            test_masks.append(test_mask)

        g.ndata['train_mask'] = torch.stack(train_masks, dim=-1).squeeze(-1)
        g.ndata['val_mask'] = torch.stack(val_masks, dim=-1).squeeze(-1)
        g.ndata['test_mask'] = torch.stack(test_masks, dim=-1).squeeze(-1)

        return g

    def __sample_split__(self, g: dgl.DGLGraph):
        ...
        if isinstance(self.num_val, float):
            num_val = round(g.num_nodes() * self.num_val)
        else:
            num_val = self.num_val

        if isinstance(self.num_test, float):
            num_test = round(g.num_nodes() * self.num_test)
        else:
        ...
        if self.split == 'train_rest':
            ...
            num_classes = int(g.ndata['label'].max().item()) + 1
            for c in range(num_classes):
                idx = (g.ndata['label'] == c).nonzero(as_tuple=False).view(-1)
